...
async function removeLocalAccount(): Promise<void> { if (!activeUser) return; if (syncStatus === "同步中") throw new Error("同步完成前不能移除此裝置資料。"); const uid = activeUser.id; conflictResolutionAbort?.abort(); profileGeneration += 1; autosync.invalidate(); activeUser = null; activeProfile = "guest"; const { error } = await supabase.auth.signOut(); if (error) throw new Error(`登出失敗：${error.message}`); await repo.deleteProfile(`user:${uid}`); await repo.deleteSyncBase(`user:${uid}`); await activateProfile("guest"); updateSyncStatus("僅本機"); }
