...
export function renderFinalizedMigrationStatus(status: MigrationStatus, data: AppData): string {
  ...
  const counts = {
    games: status.counts?.games ?? localCounts.games,
    points: status.counts?.points ?? status.counts?.review_points ?? status.counts?.reviewPoints ?? localCounts.points,
    recommendations: status.counts?.recommendations ?? status.counts?.recommended_moves ?? status.counts?.recommendedMoves ?? localCounts.recommendations,
  };
